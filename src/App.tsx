      ...
      <nav className={`fixed top-0 left-0 right-0 z-[200] h-[66px] flex items-center justify-between px-6 md:px-12 transition-all duration-300 ${
              ...
              <a 
                ...
                className={`no-underline text-[14.5px] transition-all duration-300 cursor-pointer font-medium px-4 py-2 rounded-full ${
                  (currentPage === link.id && (!link.section || link.section === 'home'))
                    ...
                    : `${isScrolled ? 'text-[#4A3B28]/80' : 'text-white/80'} hover:bg-[#C9962A]/10 hover:text-[#C9962A]`
                }`}
              >
                {link.label}
              </a>
        ...
        <button 
          className={`md:hidden flex flex-col justify-center gap-1.5 w-10 h-10 rounded-lg p-2.5 transition-all duration-300 ${
            isScrolled ? 'bg-[#C9962A]/10' : 'bg-white/10'
          }`}
          onClick={() => setIsMenuOpen(true)}
        >
          <span className={`block w-full h-0.5 rounded-sm transition-all duration-300 ${isScrolled ? 'bg-[#C9962A]' : 'bg-white'}`}></span>
          <span className={`block w-3/4 h-0.5 rounded-sm transition-all duration-300 ${isScrolled ? 'bg-[#C9962A]' : 'bg-white'}`}></span>
          <span className={`block w-full h-0.5 rounded-sm transition-all duration-300 ${isScrolled ? 'bg-[#C9962A]' : 'bg-white'}`}></span>
        </button>
      </nav>
        ...
        <div className="fixed inset-0 z-[9999] bg-black/20" onClick={() => setIsMenuOpen(false)}>
          <div className="bg-[#F5F0E8] shadow-xl animate-in slide-in-from-top duration-300" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-gold/15">
              ...
              <button onClick={() => setIsMenuOpen(false)} className="text-2xl p-2">✕</button>
            </div>
        ...
        <div className="ft-grid max-w-[1080px] mx-auto grid grid-cols-1 md:grid-cols-[2.2fr_1fr_1fr_1.5fr] gap-12 mb-12">
          <div>
            ...
            <div className="flex gap-2.5">
              <a href="#" className="w-[34px] h-[34px] border border-white/15 rounded-lg flex items-center justify-center text-sm text-white/50 no-underline transition-all hover:bg-gold hover:border-gold hover:text-white">f</a>
              <a href="#" className="w-[34px] h-[34px] border border-white/15 rounded-lg flex items-center justify-center text-sm text-white/50 no-underline transition-all hover:bg-gold hover:border-gold hover:text-white">📷</a>
              <a href="#" className="w-[34px] h-[34px] border border-white/15 rounded-lg flex items-center justify-center text-sm text-white/50 no-underline transition-all hover:bg-gold hover:border-gold hover:text-white">in</a>
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <ul className="list-none flex flex-col gap-2.5 p-0">
              <li><a href="#" className="no-underline text-[13px] text-white/55 transition-colors hover:text-gold-light">Join the Network</a></li>
              <li><a href="#" className="no-underline text-[13px] text-white/55 transition-colors hover:text-gold-light">Painter Plans</a></li>
              <li><a href="#" className="no-underline text-[13px] text-white/55 transition-colors hover:text-gold-light">Painter Dashboard</a></li>
              <li><a href="#" className="no-underline text-[13px] text-white/55 transition-colors hover:text-gold-light">Contact Us</a></li>
            </ul>
          </div>
          <div>
            ...
            <div className="text-[13px] flex flex-col gap-2.5">
              <div className="flex items-center gap-1">
                ...
                <a href="mailto:brushd.au@gmail.com" className="text-white/75 no-underline">brushd.au@gmail.com</a>
              </div>
              <a href="#" className="text-white/30 text-[11px] mt-2">Privacy Policy</a>
              <a href="#" className="text-white/30 text-[11px]">Terms of Service</a>
            </div>
          </div>
        </div>
